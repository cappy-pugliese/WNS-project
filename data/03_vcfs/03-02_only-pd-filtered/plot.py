...
#   plot-vcfstats -p /home/FCAM/cpugliese/lab_wns/05_vcfs/02_stats/03_only-pd/01_bcfplots/01_only-pd/02_after-filtering only-pd_filtered_bcftools_stats.vchk
#
# Edit as necessary and recreate the plots by running
#   python3 plot.py
#
# Title abbreviations:
# 	 0 .. only- .. /home/FCAM/cpugliese/lab_wns/05_vcfs/03_filtered-vcfs/only-pd-ploidy1-filtered.vcf.gz
#
...
if 0 in dat and len(dat[0])>2:
    ax1.plot([row[0] for row in dat[0]], [row[1] for row in dat[0]], '-o',markersize=3, color='orange',mec='orange',label='only-')
    has_data = 1
